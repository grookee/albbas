...
export async function serveUploadBySlug(
  slug: string,
  ...
  const download = (request.query as { dl?: string }).dl === '1';
  const isInline =
    !download && INLINE_PREFIXES.some((prefix) => upload.mimeType.startsWith(prefix));
