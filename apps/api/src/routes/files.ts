...
import { getUploadCache, setUploadCache, type UploadCacheEntry } from '../lib/cache.js';
import { storage } from '../storage/instance.js';

const SLUG_PATTERN = /^[A-Za-z0-9]{9}$/;

const INLINE_PREFIXES = ['image/', 'video/', 'audio/', 'application/pdf', 'text/plain'];
...
async function serveFile(
  request: FastifyRequest<{ Params: { slug: string }; Querystring: { dl?: string } }>,
  reply: FastifyReply,
): Promise<FastifyReply | undefined> {
  const { slug } = request.params;
  if (!SLUG_PATTERN.test(slug)) return reply.code(404).type('text/plain').send('404: not found');
  ...
  const isInline = !download && INLINE_PREFIXES.some((prefix) => upload.mimeType.startsWith(prefix));
