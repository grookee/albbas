...
async function generateUniqueSlug(): Promise<string> {
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    const existing = await prisma.upload.findUnique({ where: { slug }, select: { id: true } });
...
async function handleUpload(
  ...
  const putPromise = storage.put(s3Key, { stream: counter, contentType: mimeType });
  ...
  await prisma.upload.create({
    data: { slug, userId: user.id, originalName, mimeType, sizeBytes, s3Key, baseUrl },
  });
